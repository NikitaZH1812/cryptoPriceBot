...
def start(message):
    bot.send_message(message.chat.id, "If you want get a price, enter crypto pair in format like BTCUSDT")
...
def get_user_text(message):
    ...
    if ms.upper() in subols_list:
        ...
        bot.send_message(message.chat.id, get_price(f"{crypto_pair_with_usdt}"))
    else:
        bot.send_message(message.chat.id, "sorry, we have no information about this crypto")
